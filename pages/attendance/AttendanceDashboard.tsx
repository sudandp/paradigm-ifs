...
interface MonthlyReportRow {
     ...
     * 'P'    – Present (full day)
     * '0.5P' – Half Day Present
     * 'W/O'  – Week Off
     * 'W/P'  – Week Off Present
     * 'W/H'  – Work from Home
     * 'E/L'  – Earned Leave
     * 'S/L'  – Sick Leave
     * 'C/O'  – Comp Off
...
const AttendanceDashboard: React.FC = () => {
    ...
    const [reportType, setReportType] = useState<'basic' | 'log' | 'monthly' | 'audit'>('basic');
    ...
    const pdfContent = useMemo(() => {
        if (reportType === 'basic') {
            ...
            return <AttendanceLogPdfComponent data={attendanceLogData} dateRange={dateRange} />;
                ...
                return <AttendanceAuditReport logs={auditLogs} />;
            }
            return null;
        }, [reportType, basicReportData, attendanceLogData, monthlyReportData, dateRange, auditLogs]);
    ...
    const handleDownloadCsv = async () => {
        ...
        try {
            if (reportType === 'monthly') {
                // For monthly report, we now use the formatted Excel export
                ...
                if (logo && logo.startsWith('data:image')) {
                    logoBase64 = logo;
                } else if (logo && (logo.startsWith('http') || logo.startsWith('/'))) {
                    try {
                        ...
                        // If fetching fails, we leave logoBase64 empty so the utility skips it gracefully
                    }
                }
                ...
                // Fallback to legacy CSV for other report types for now
                ...
                if (reportType === 'basic') {
                    ...
                    // Header
                    csvContent += "Date & Time,Action,Performed By,Target Employee,Details\n";
                    // Data
                    auditLogs.forEach(log => {
                        const detailsStr = JSON.stringify(log.details).replace(/"/g, '""'); // Escape quotes
                        csvContent += `"${format(new Date(log.created_at), 'yyyy-MM-dd HH:mm')}","${log.action}","${log.performer_name}","${log.target_name}","${detailsStr}"\n`;
                    });
                }
