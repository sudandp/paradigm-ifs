...
const RoleManagement: React.FC = () => {
    ...
    const renderPermissionRow = (permInfo: { key: Permission; name: string; description: string; }) => (
        <tr key={permInfo.key}>
            ...
            {roles.map(role => {
                const isRoleAdmin = isAdmin(role.id);
                const isCurrentUserRole = user?.role === role.id;
                const isCorePermission = permInfo.key === 'manage_roles_and_permissions';
                const isChecked = isRoleAdmin || (permissions[role.id]?.includes(permInfo.key));
                const isDisabled = isRoleAdmin || (isCurrentUserRole && isCorePermission);
