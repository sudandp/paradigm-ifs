...
import type {
  ...
  BiometricDevice, ChecklistTemplate, FieldReport
} from '../types';
...
export const api = {
  ...
  getLocationHistory: async (userId: string, date: string): Promise<AttendanceEvent[]> => {
    const start = `${date}T00:00:00Z`;
    const end = `${date}T23:59:59Z`;
    return api.getAttendanceEvents(userId, start, end);
  },
  ...
  addBiometricDevice: async (device: Partial<BiometricDevice>): Promise<BiometricDevice> => {
    const { data, error } = await supabase.from('biometric_devices').insert(toSnakeCase(device)).select().single();
    if (error) throw error;
    return toCamelCase(data);
  },
  ...
  updateBiometricDevice: async (id: string, device: Partial<BiometricDevice>): Promise<BiometricDevice> => {
    const { data, error } = await supabase.from('biometric_devices').update(toSnakeCase(device)).eq('id', id).select().single();
    if (error) throw error;
    return toCamelCase(data);
  },
