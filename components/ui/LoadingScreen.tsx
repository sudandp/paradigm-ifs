...
const LoadingScreen: React.FC<LoadingScreenProps> = ({ message = 'Loading...', fullScreen = true }) => {
    ...
    const generateStreaks = (n: number, color: string) => {
        let value = '';
        for (let i = 0; i < n; i++) {
    ...
    const containerStyle = fullScreen 
        ? "fixed inset-0 z-[9999] overflow-hidden bg-gradient-to-br from-[#0a0b1e] via-[#050505] to-[#0a0b1e] flex items-center justify-center font-['Inter',_sans-serif]"
        : "relative overflow-hidden bg-gradient-to-br from-[#0a0b1e] via-[#050505] to-[#0a0b1e] flex flex-col items-center justify-center min-h-[400px] w-full font-['Inter',_sans-serif] rounded-xl shadow-2xl transition-all duration-300";
    ...
    return (
        <div className={containerStyle}>
            ...
            <div className="text-center z-10 flex flex-col items-center">
                <div className={`rocket-wrapper ${fullScreen ? 'mb-8' : 'mb-4'} relative flex justify-center items-center ${fullScreen ? 'h-64' : 'h-48'}`}>
                    <div className="rocket-container">
                        <svg width={fullScreen ? "180" : "140"} height={fullScreen ? "240" : "180"} viewBox="0 0 40 80" fill="none" className="rocket-body overflow-visible">
                            <defs>
                ...
                <div className={`${fullScreen ? 'px-6' : 'px-4'} relative z-30`}>
                    <h1 className={`${fullScreen ? 'text-4xl' : 'text-2xl'} font-black text-white mb-2 tracking-[0.2em] italic uppercase drop-shadow-[0_0_15px_rgba(255,255,255,0.3)]`}>
                        {message === 'Loading...' ? 'Ignition' : message}
                    </h1>
                    <div className="flex flex-col items-center gap-4">
                        <p className="text-blue-400 text-xs font-mono tracking-widest animate-pulse">
                            &gt; Establishing secure uplink...
                        </p>
                        <div className={`${fullScreen ? 'w-72' : 'w-48'} h-1 bg-gray-900/50 backdrop-blur-sm rounded-full overflow-hidden border border-blue-900/30`}>
            ...
            {/* Subtle nebula glows */}
            <div className={`absolute top-1/4 left-1/4 ${fullScreen ? 'w-[40rem] h-[40rem]' : 'w-[20rem] h-[20rem]'} bg-blue-600/10 rounded-full blur-[120px] pointer-events-none animate-pulse`}></div>
            <div className={`absolute bottom-1/4 right-1/4 ${fullScreen ? 'w-[30rem] h-[30rem]' : 'w-[15rem] h-[15rem]'} bg-purple-600/10 rounded-full blur-[120px] pointer-events-none animate-pulse`} style={{ animationDelay: '1s' }}></div>
        </div>
    );
};
